...
After agents return, use AskUserQuestion before synthesizing. Summarize the key finding in a sentence or two, then surface anything unexpected and ask the user to react. Present specific choices about how to proceed — don't just ask "does this make sense?" **Explain each option's tradeoffs in plain, jargon-free terms — ELI5 style.**
...
- **Every option presented in AskUserQuestion must include an ELI5 explanation of its tradeoffs. Never assume the user understands the underlying technical details.**
